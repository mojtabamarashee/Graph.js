...
class Graph extends Component {
  ...
  showAxes() {
    ...
    for (i = 1; i <= this.state.AXES_STEP_NUM_Y; i++) {
      ...
      ptr.ctx1.lineTo(
        ptr.canvas1.width - this.global.padding.x.right,
        ptr.canvas1.height - this.global.padding.y.up - ptr.scaleY * i * step
      )
    }
    ...
    for (i = 0; i <= ptr.AXES_STEP_NUM_X; i++) {
      ...
      //zaedehaye x
      ptr.ctx1.lineTo(
        this.global.padding.x.left + ptr.scaleX * i * step,
        ptr.canvas1.height - this.global.padding.y.down + this.global.padding / 4
      )
      ...
      if (showRotatedX == 1) {
        ...
        ptr.ctx1.translate(
          this.global.padding.x.left + ptr.scaleX * i * step,
          ptr.canvas1.height - this.global.padding.y.up + 10
        )
        ...
        ptr.ctx1.fillText(
          xl,
          this.global.padding.x.left + ptr.scaleX * i * step - w / 2,
          ptr.canvas1.height - this.global.padding.y.up + this.global.X_LABEL_PADDING
        )
      }
      ptr.ctx1.restore()
    }
    ...
    for (i = 0; i <= this.state.AXES_STEP_NUM_Y; i++) {
      ...
      if (showScien == 1) {
        ...
        ptr.ctx1.fillText(
          base + 'e' + ans.power,
          this.global.padding.x.left - w - 10,
          ptr.canvas1.height - this.global.padding.y.down - ptr.scaleY * i * step
        )
      } else {
  ...
  MouseMove(e) {
    if (this.global.downFlag == 1) {
      if (this.global.panFlag == 1) {
        ...
        this.global.ctx2.rect(
          this.global.startX,
          this.global.startY,
          mouseX - this.global.startX,
          mouseY - this.global.startY
        )
  ...
  MouseUp(e) {
    if (this.global.downFlag == 1 && this.global.mouseMove == 1) {
      ...
      var zoomStartY =
        (this.global.canvas3.height - this.global.padding.y.down - this.global.startY) / this.global.scaleY +
        this.global.minY
      var zoomEndY =
        (this.global.canvas3.height - this.global.padding.y.down - (parseInt(e.clientY) - rect.top)) /
          this.global.scaleY +
        this.global.minY
      ...
      if (zoomEndX - zoomStartX > 1) {
        ...
        this.global.scaleX =
          this.global.canvas1.width / sxx - (this.global.padding.x.left + this.global.padding.x.right) / sxx
        ...
        this.global.xPlot = this.global.xPlot.slice(
          this.global.sxIndex - prevSxIndex,
          this.global.exIndex - prevSxIndex + 1
        )
  ...
  FindMinMax(ptr, min, max) {
    ...
    if ((maxY < 1 && (maxY > -1) & (maxY < 1) && maxY > -1) || (maxY - minY > -1 && maxY - minY < 1)) {
      ss = Math.min(
        Math.floor(Math.log10(Math.abs(maxY))),
        Math.floor(Math.log10(Math.abs(maxY))),
        Math.floor(Math.log10(maxY - minY))
      )
      ss = -ss
  ...
  Plot() {
    ...
    if (this.state.pauseFlag == 0 || this.state.forcePlot == 1) {
      ...
      this.global.ctx1.fillText(
        this.props.title,
        this.global.canvas1.width / 2 - w / 2,
        (2 * this.global.padding.y.up) / 3
      )
      this.global.ctx1.stroke()
      this.global.ctx1.closePath()
      ...
      if (this.state.zoomFlag == 0) {
          ...
          //if(this.settings.autoScaleFlag == true)
          {
            ...
            if (this.global.minY != this.global.maxY) {
              ...
              if (Math.abs(this.global.minY) % d != 0) {
                if (this.global.minY < 0) this.global.minY = this.global.minY + (Math.abs(this.global.minY) % d) - d
                else this.global.minY = this.global.minY - (this.global.minY % d)
              }
        ...
        if (Math.abs(this.global.minX) % d != 0) {
          if (this.global.minX < 0) this.global.minX = this.global.minX + (Math.abs(this.global.minX) % d) - d
          else this.global.minX = this.global.minX - (this.global.minX % d)
        }

        if (this.global.maxX % d != 0) this.global.maxX = this.global.maxX + d - (this.global.maxX % d) // mihahim maxX mazrabe d bashad
        ...
        this.global.scaleX =
          this.global.canvas1.width / sx - (this.global.padding.x.left + this.global.padding.x.right) / sx
    ...
    return (
      <div>
        <div style={divStyle}>
          ...
          />
          <canvas id="canvas4" ref="canvas4" style={Style} />
          <canvas id="canvas3" ref="canvas3" width={this.props.width} height={this.props.height} style={Style} />
          <canvas id="canvas2" ref="canvas2" width={this.props.width} height={this.props.height} style={Style} />
        </div>
        <Tools
          ZoomOut={this.ZoomOut}
          pauseFlag={this.state.pauseFlag}
          TogglePause={this.TogglePause}
          ToggleDotMode={this.ToggleDotMode}
          ToggleMaxHFlag={this.ToggleMaxHFlag}
          ToggleAvgFlag={this.ToggleAvgFlag}
          ToggleAvgFlag={this.ToggleAvgFlag}
          AVGNumSet={this.AVGNumSet}
        />
      </div>
    )
  }
